...
from common import DAY_OFF_COLOR
...
class Work_Time(ttk.LabelFrame):
        ...
        for column in range(1, self.t_width):
            for row in range(1, self.t_height):
                ...
                if column == 1:
                    ...
                    self.entries[row, column].bind(
                        "<Return>",
                        combine_funcs(make_lambda(row), make_save(), focus_next_widget),
                    )
                    self.entries[row, column].grid(
                        row=row,
                        column=column,
                    )
                elif column <= 4:
                    ...
                    self.entries[row, column] = ttk.Entry(
                        self, width=10, justify="center"
                    )
                    self.entries[row, column].bind(
                        "<Return>", combine_funcs(make_save(), focus_next_widget)
                    )
                    self.entries[row, column].grid(row=row, column=column)
                else:
                    self.entries[row, column] = ttk.Entry(
    ...
    def calculate(self, row):
        ...
        def f6():
            for col in range(3, 10):
                self.entries[row, col].delete(0, 10)
            if self.labels[row].cget("background") == DAY_OFF_COLOR:
    ...
    def make_month(self, date):
        ...
        for row in range(1, calendar.monthrange(date.year, date.month)[1] + 1):
            self.labels[row] = ttk.Label(
                self, text=self.master.input_table.labels[row]["text"]
            )
            self.labels[row].config(
                background=self.master.input_table.labels[row].cget("background"),
                anchor="center",
            )

            self.labels[row].grid(row=row, column=0, sticky="ew")

        # if os.path.isfile(str(SCHEDULES_DIR) + str(date.year) + "/" + str(date.month) + ".txt"):
        #     with open(str(SCHEDULES_DIR) + str(date.year) + "/" + str(date.month) + ".txt", "r") as f:
